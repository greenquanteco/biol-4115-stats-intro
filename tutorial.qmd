...
#| fig-width: 5
#| fig-height: 4
#| fig-cap: "Example boxplot from a simulated study showing difference in parasite count between adult and juvenile fish."
#| fig-alt:  "Example boxplot from a simulated study showing difference in parasite count between adult and juvenile fish."
#| label: fig-exampleboxplot
...
#| fig-width: 5
#| fig-height: 4
#| fig-cap: "Example scatterplot from a simulated study showing a positive correlation between parasite count and body length in fish."
#| fig-alt:  "Example scatterplot from a simulated study showing a positive correlation between parasite count and body length in fish."
...
```{r}
#| fig-cap: "Illustration of normal distributions with different means and standard deviations."
#| fig-alt:  "Illustration of normal distributions with different means and standard deviations."
#| label: fig-examplenormal
...
```{r}
#| fig-cap: "As sample size increases, the estimate of the population mean becomes more and more certain."
#| fig-alt:  "As sample size increases, the estimate of the population mean becomes more and more certain."
#| label: fig-estimatingmusigma
#| 
...
If your data do not meet the assumptions for ANOVA, then you can use a **Kruskal-Wallis test**. This is sometimes called a "nonparametric ANOVA" but that isn't quite accurate. Essentially, the Kruskal-Wallis test compares the relative rank order of observations between groups to test whether values in each group tend to be consistently larger than or smaller than other groups. 

```{r}
# run the Kruskal-Wallis test
mod4 <- kruskal.test(Petal.Length~Species, data=iris)
mod4
```

Similar to what happened in the one-way ANOVA example above, the omnibus test was significant, so we need to run a post-hoc test. The most common post-hoc test for a Kruskal-Wallis test is the **Dunn test**, available in package `dunn.test`.
...
Now, we can perform the Dunn's test to see which groups likely differ from each other, much like we used the Tukey HSD test above.

```{r}
dunn.test(iris$Petal.Length, g=iris$Species, method="holm")
```

The output shows, for each pair of factor levels, the differences in median ranks between groups (the top number), and the adjusted *p*-value for each comparison. The *p*-value is adjusted for multiple comparisons. In this case, we used a [Holm correction](https://en.wikipedia.org/wiki/Holm%E2%80%93Bonferroni_method).
...
This notion of linearity is what makes many analyses with factors (categories or grouping variables) as predictors “linear”. For example, the *t*-test is a linear model, but it doesn’t really describe a line on a plot of $y$ vs. $x$ the way that linear regression does. However, if you encode the factor (grouping variable) as 0s and 1s and think about the underlying algebra, it becomes clear what the difference in group means really means:
...
![Illustration of why a t-test is a linear model.](tut01.jpg){#fig-linear1 fig-cap="Why a *t*-test is a linear model. Changing groups (the predictor varible) from 0 to 1 increases the expected value of Y by $beta_1$, or the difference in group means."}
 
Similar logic explains why ANOVA with 3 or more levels is also a linear model.

## Assumptions of linear models

As powerful and useful as linear models can be, they are not without assumptions that must be met. There are 4 assumptions that your data must meet before using linear models:

**Normal residuals:** The linear model assumes that model residuals, the random errors representing the differences between observed and predicted values, follow a normal distribution. Models for data that do not have normal residuals exist, but they are not linear models. 

**Homoscedasticity, or constancy of variance:** Linear models assume **homoscedasticity**, or constant variance. This means that the residual variance does not depend on either $X$ or $Y$. This is part of the definition of a variable or a residual being independent and identically distributed (i.i.d.). If the variance in the response variable depends on some predictor variable, then this should be incorporated into the model (resulting in something other than a linear model). If the variance appears to depend on the response variable, then there is probably an issue with the assumed response distribution and you should consider using GLMs (below) or mixed models (outside the scope of this tutorial).

Here is what homoscedasticity and heteroscedasticity (unequal variance) look like in a scatterplot:

![Illustration homoscedasticity and heteroscedasticity in a scatterplot.](tut02.jpg){#fig-homoscedasticity fig-cap="Illustration homoscedasticity and heteroscedasticity in a scatterplot. On the left, the variance is roughly constant throughout the domain of $X$. On the right, residual variance increases as $X$ increases."}

Here is what they look like in a boxplot.

![Illustration of homoscedasticity and heterscedasticity in a boxplot.](tut03.jpg){#fig-homoscedasticity2 fig-cap="Illustration homoscedasticity and heteroscedasticity in a scatterplot. On the left, the variance is roughly the same in both groups as shown by the width of the boxes and reaches of the whiskers. On the right, Group 2 has much more variance than Group 1."}
