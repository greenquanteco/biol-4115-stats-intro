...
# Why do we have to do statistics? {#sec-why}
...
# Types of tests {#sec-typesoftests}
...
The hypotheses that we test in biology, and thus the statistical tests we perform, tend to fall into 4 categories:

1.  Testing for a difference in **mean** or **location** between groups.

    - If you have 2 groups, use a *t*-test (see @sec-groups-ttest).
  
    - If you have 3 or more groups, use analysis of variance (ANOVA; see @sec-groups-anova).

2.  Testing for a **continuous relationship** between a **response variable** and one or more **predictor variables**. Also referred to as predicting or **modeling** a response variable.

    - If you don't need a prediction equation test for a correlation (see @sec-correlation)
  
    - If you wnat to model continuous $y$ as a function of one or more continuous $x$ variables, and the relationship is linear, try linear regression (see @sec-regression-regression). 
  
    - If you want to model continuous $y$ as a function of a continuous predictor $x$ and a factor predictor (grouping or categorical variable), try analysis of covariance (ANCOVA) (see @sec-regression-ancova).
    
    - If you want to model **count data**, you should use a Poisson GLM (@sec-glm-poisson).
    
    - If you want to model proportions, you need a binomial GLM (@sec-glm-binomial).

3.  Testing for (lack of) **independence between categories** of observations.

    - You have very small sample sizes: try a Fisher's exact test (see @sec-contingency-fisher).
    
    - You have moderate to large sample size: try a $\chi^2$ test ("chi-squared") (see @sec-contingency-chisq). 

4.  **Classifying** observations based on predictor variables.

    - For binary classification, start with logistic regression (see @sec-glm-logistic).
    
    - For $>2$ categories, try classification and regression trees (CART). Beyond the scope of this tutorial, but see package `rpart` and its documentation.

These categories are not mutually exclusive...some questions can be analyzed in $>1$ way!

# Summary statistics {#sec-summary}

## Summarizing single variables {#sec-summary-single}
...
## Summarizing by groups in R {#sec-summary-agg}
...
aggregate(Petal.Length~Species, data=iris, 
          function(z){length(which(is.na(z)))})
```
...
## Comparing 2 groups: the *t*-test {#sec-groups-ttest}
...
## Comparing 3 or more groups: ANOVA {#sec-groups-anova}

### One-way ANOVA {#sec-groups-anova-1way}
...
### Two-way ANOVA (and more ways) {#sec-groups-anova-2way}
...
If you analyze an ordered factor, R will estimate polynomial terms of order up to $k-1$, where $k$ is the number of factor levels. Let's create a toy dataset to illustrate working with an ordered factor.
...
The omnibus test shows us that `age` is significant. However, because we have an ordered factor, we don't want something like a Tukey test. Instead, we want to see what patterns in `age` are significant.
...
This analysis was simple because there were only 3 levels of `age`. When an ordered factor has many levels, you may observe "significant" polynomial trends that are biologically unimportant or difficult to interpret. It is up to you as a biologist to exercise some judgement in deciding how many terms to interpret.

## Interactions: one variable affects another {#sec-groups-interactions}

Something we could also ask is whether there was an **interaction** between species and color: a situation where one variable changes the effect of another. Maybe purple and pink flowers really do have different petal lengths, but only in one species. Testing for an interaction would help detect that kind of pattern. To test for an interaction between two variables, use the `*` symbol instead of the `+`.
...
## Linear correlation {#sec-correlation-linear}
...
## Nonlinear correlation {#sec-correlation-nonlinear}
...
**Why it's linear:** although the graph may be curved, the parameters $\beta_0$, $\beta_1$, $\beta_2$, and so on appear only as multipliers on predictor variables, are combined by addition, and are not raised to powers, exponentiated, or use inside other functions.
...
![Rick and Morty meme saying this sounds like linear with extra steps.](tut05.jpg){#fig-extrasteps fig-cap="Well that sounds like linear with extra steps."}
...
In contrast, scientists and engineers often use other bases, like $log_{10}$ or $log_2$, because these are more intuitive for humans. You can use whatever base you like so long as you are clear about what you did.
:::
...
## Linear regression {#sec-regression-regression}
...
## Multiple linear regression {#sec-regression-multiple}
...
## Analysis of covariance (ANCOVA) {#sec-regression-ancova}
...
## Models for counts: Poisson GLM {#sec-glm-poisson}
...
We can get a good sense of how well the model fits the data by calculating a pseudo-$R^2$ as $\left(Dev_{null}-Dev_{res}\right)/Dev_{null}$, or in this case $(179.544-65.133)/179.544=0.637$. The AIC, 302.62, doesn't mean much on its own but is useful for comparing models; generally, models with smaller AIC are better fit to the data.
...
## Models for binary outcomes: Logistic Regression {#sec-glm-logistic}
...
For logistic regression, we can either use pseudo-$R^2$ to measure goodness-of-fit, or we can use a statistic called **AUC** to measure how well the model discriminates between observations where $Y=1$ and those where $Y=0$.

The **receiver operating characteristic (ROC) curve** summarizes the trade-off between sensitivity (true positive rate) and specificity (true negative rate) for a logistic regression model. The area under the ROC curve, or **AUC**, is a value that ranges from about 0.5 (as accurate as a coin flip) to 1 (perfect sensitivity and specificity). It is available in the package `pROC`. 
...
In order to get the AUC, we need to first get the model predictions and then compare those to the original data.
...
The AUC of this model, 0.942, means that the model is very good at distinguishing observations where $Y=1$ from observations where $Y=0$. Many people interpret AUC as a measure of predictive accuracy, but what it really does is measure how well the model orders the probabilities of $Y=1$ for each observation. AUC is the probability that a randomly chosen positive observation will receive a higher score than a randomly chosen negative observation. It does not tell us the absolute probability of a prediction being correct; a model could have high AUC even if its predicted probabilities were poorly calibrated.

The take-home message is that AUC is one useful tool in evaluating the performance of a logistic regression model, but, like $R^2$ for linear models or AIC for models in general, it represents but one perspective among many.
...
## Models for proportions: Binomial Regression {#sec-glm-binomial}
...
## Gamma GLMs for overdispersed continuous data  {#sec-glm-gamma}

Coming soon!

# Tests for contingency {#sec-contingency}
...
## Fisher's exact test {#sec-contingency-fisher}

**Fisher’s exact test** is a test for independence in contingency tables that is usually only used when sample sizes are small or when the $\chi^2$ test gives questionable approximations.
...
## Chi-squared test {#sec-contingency-chisq}
